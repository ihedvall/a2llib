...
enum class UdpPacketAlignment : uint8_t {
  PACKET_ALIGNMENT_8 = 0,
  PACKET_ALIGNMENT_16 = 1,
  PACKET_ALIGNMENT_32 = 2
};

enum class UdpSubCmd : uint8_t {
  GET_SLAVE_ID = 0xFF,
...
using UdpSubCmdList = std::vector<UdpSubCmd>;

class XcpOnUdpIp : public Protocol {
public:
  ...
  [[nodiscard]] UdpPacketAlignment GetPacketAlignment() const { return packet_alignment_; }

  void AddUdpSubCmd(const std::string& cmd);
  [[nodiscard]] const UdpSubCmdList& GetUdpSubCmds() const {
...
private:
  ...
  UdpPacketAlignment packet_alignment_ = UdpPacketAlignment::PACKET_ALIGNMENT_8;
  UdpSubCmdList optional_tl_subcommand_list_;
