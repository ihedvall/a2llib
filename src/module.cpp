...
void CheckFlatMapList(
  ...
  const a2l::FilterList& filter_list, std::vector<T*>& dest_list) {
  if (!filter_list.IsModified()) {
    return;
  }

  try {
    dest_list.clear();
    dest_list.reserve(source_list.size());

    for (const auto& [name, source_item] : source_list) {
      if (filter_list.IsMatchingFilter(*source_item)) {
        dest_list.emplace_back(source_item.get());
      }
    }
    std::ranges::sort(dest_list, [] (const T* lhs, const T* rhs) -> bool {
...
std::vector<std::string> Module::SearchCharacteristics(
  ...
  dest_list.reserve(characteristic_list_.size());
  for (const auto& [name, characteristic] : characteristic_list_) {
    if (filter.IsMatchingFilter(*characteristic)) {
      dest_list.emplace_back(name);
    }
  }
  std::ranges::sort(dest_list);
  return dest_list;
}
...
Characteristic* Module::GetFlatCharacteristic(long index) const {
  CheckFlatMapList(characteristic_list_, characteristic_filter_,
    flat_characteristic_list_);
...
const FlatCharacteristicList& Module::GetFlatCharacteristicList() const {
  CheckFlatMapList(characteristic_list_, characteristic_filter_,
  flat_characteristic_list_);
  return flat_characteristic_list_;
}
...
Measurement* Module::GetFlatMeasurement(long index) const {
  CheckFlatMapList(measurement_list_, measurement_filter_,
    flat_measurement_list_);
...
const FlatMeasurementList& Module::GetFlatMeasurementList() const {
  CheckFlatMapList(measurement_list_, measurement_filter_,
    flat_measurement_list_);
  return flat_measurement_list_;
}

std::vector<std::string> Module::SearchMeasurements(
    const FilterList& filter) const {
  std::vector<std::string> dest_list;
  dest_list.reserve(measurement_list_.size());
  for (const auto& [name,measurement] : measurement_list_) {
    if (filter.IsMatchingFilter(*measurement)) {
      dest_list.emplace_back(name);
    }
  }
  std::ranges::sort(dest_list);
